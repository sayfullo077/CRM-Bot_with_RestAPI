...
from keyboards.inline.inline_buttons import back_button, category_menu_button
from loader import dp


@dp.callback_query(F.data.startswith("category_menu"), ShopState.mein_menu)
async def shop_menu(call: types.CallbackQuery, state: FSMContext):
    await state.set_state(ShopState.category_menu)
    telegram_id = call.from_user.id
    async with ClientSession() as session:
        user = await get_user(telegram_id=telegram_id, session=session)
        language = user.get("language")
        shop_menu_btn = await category_menu_button(language)
